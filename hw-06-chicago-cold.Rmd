...
Below is the code we used to create the `convert_temperature()` function.
Do **not** copy it into the homework 6 starter file.
Instead, we are going to save our function code in an R script.

1.  Open a new `R Script` (under File \> New File \> R Script) and copy the code to create the function into scripting window. Exclude the top and bottom lines of the chunk (i.e. the lines that start with three back-ticks). Save the R script file as "convert_temperature.R" in the same directory as your starter file .Rmd. Finally, copy the code chunk below titled `import-convert-temperature` into Exercise 1 in your starter file. You will need to change `eval = T`.
...
We just created an R script containing the code for `convert_temperature()` and saved it with the name `convert_temperatures.R`.
When we run the function `source("convert_temperatures.R")`, R executes the code in that R script in our RMarkdown environment and, as a result, creates the function object `convert_temperature()`.

2.  Just as as sanity-check, let's test out the function we just imported. Insert a new labeled code chunk below Exercise 2 in your starter file. Write a single line of code that will test your function with the single Fahrenheit value = `32`.

## Importing and recoding the data

3.  Look at the 3 `.csv` files in your starter directory `/data` folder.
    Insert a labeled code chunk under Exercise 3 in your starter file that imports each of the three datasets and assigns them to object names of your choice.
...
4.  Notice that the `date` column is still as data type `character string`.
    ...
    **Then** add a second labeled chunk below the first, that gives you a glimpse of each dataset to confirm that they now have the correct `date` column data type.

If you are struggling with the col_type argument, you can check the syntax in Week 9 slides on Blackboard `importing and recoding data`.

## Wrangling and merging the data

5.  Copy the code chunk below into Exercise 5 in your starter file. Complete it so that each data set is changed in the following ways: 1) `mutate()` is used to create a new column called `neighborhood`; 2) `relocate()` is used to move the new column to the first column position; 3) then, in the fourth pipeline, each data set is joined using `bind_rows()` to create a new data frame object called `chicago_temperatures`. Remember to change `eval = T`.
...
6.  Let's make a plot to visualize the data and determine which neighborhood was typically the coldest in February 2021.
    Insert and label a code chunk below Exercise 6 in your starter file that takes `chicago_temperatures` as the input dataset, and maps `date` on the x-axis, `temperature_high` on the y-axis, and maps `neighborhood` to the `color` aesthetic.
...
`map()` iterates functions over elements in a list, however, our data is currently in a data frame.

8.  Copy and complete the code chunk below into Exercise 8 in your starter file. Complete it to create a list of length 3 from the 3 temperature columns in the `chicago_temperatures` data frame object. Remember to change `eval = T`.
...
9.  Write and label a code chunk under Exercise 9 in your starter file that will apply the `convert_temperature()` function to each vector element in your new list object and will combine output together **by row** into a dataframe.
